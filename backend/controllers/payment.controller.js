...
metadata: {
    ...
    products: JSON.stringify(
        products.map((p) => ({
         ...
         res.status(200).json({id: session.id, totalAmount: totalAmount / 100, url: session.url});//redirect to checkout page url
...
export const checkoutSuccess = async (req,res) => {
    try{
        ...
        if(session.payment_status === "paid"){
            ...
            // start of clear cart on backend
            req.user.cartItems = [];
            await req.user.save();
            // end of clear cart on backend
